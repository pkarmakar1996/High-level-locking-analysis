...
int KeySchedule(int type, int key[32], int int_keys[17], _Bool bool_keys[81])
{
	...
	int nk, nb, round_val;
				...
				for (j = nk; j < nb * (round_val + 1); ++j)
				{
					...
					{
						if(int_keys[0] == 0) temp[0] = word[0][j - 1];
						else temp[0] = word[1][j-1];

						if(int_keys[1] == 0) temp[1] = word[1][j - 1];
						else temp[1] = word[0][j-1];

						if(int_keys[2] == 0) temp[2] = word[2][j - 1];
						else temp[2] = word[3][j-1];

						if(int_keys[3] == 0) temp[3] = word[3][j - 1];
						else temp[3] = word[2][j-1];
